define(function(require){
	...
	o.indexAction=function() {
		...
		var code = '<div class="mwt-dialog" style="display:block;top:69px;">'+
          '<div class="dialog-body bounceInDown" style="top:50px; width:400px; display:inline-block;">'+
...
/*                '<tr height="45"><td>验证码：</td>'+
          ...
          '</div>'+
        '</div>'+
		'<div style="position:absolute;left:0;right:0;top:320px;z-index:99;">'+
...
*/
	...
	function logsub() {
		...
		params.username = mwt.get_text_value('username-divtxt');
		params.userpass = mwt.get_text_value('userpass-divtxt');
		jQuery('#logbtn').unbind('click').html('登录中...');
		//print_r(params);
		ajax.post('member&action=login',params,function(res){
